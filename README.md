...
### Optional NVIDIA Audio2Face animation

Direct TTS and Voice Conversation can animate a validated user-supplied 3D GLB from the 52 facial
controls generated by NVIDIA Audio2Face from the same Piper PCM. Three.js renders the face locally;
the existing SVG remains the visible fallback. Normal streaming Piper remains available when
Audio2Face is not installed. Audio2Face v1
is Windows x64/NVIDIA GPU only and requires Visual C++ Build Tools, Python 3.10, CUDA 12.8/12.9,
TensorRT 10.13, Git LFS, accepted NVIDIA/model terms, and Hugging Face authentication.

Keep the official SDK checkout at `Audio2Face-3D-SDK` and at the pinned commit expected by the setup
script, then validate without downloads:

```powershell
scripts\setup-audio2face.ps1 -ValidateOnly
```

After installing CUDA/TensorRT and accepting access to the required NVIDIA models, run:

```powershell
scripts\setup-audio2face.ps1 -AcceptNvidiaSdkLicense -AcceptModelLicenses
```

Then set `HARNESS_AUDIO2FACE_ENABLED=true` and restart the browser server. Setup installs protected
runtime artifacts under `.harness`; application startup and first use never download anything.

Install a licensed, self-contained GLB containing all canonical 52 ARKit morph targets:

```powershell
scripts\setup-audio2face-avatar.ps1 -AssetPath C:\path\avatar.glb -ConfirmAssetRights
```

Install additional characters under exact IDs without replacing the current default; both Direct
TTS and Voice Conversation then show a **Character** switch:

```powershell
scripts\setup-audio2face-avatar.ps1 -AssetPath C:\path\amber.glb `
  -AvatarId amber -DisplayName "Amber Presenter" -ConfirmAssetRights
```

For a rights-confirmed Character Creator RAR containing `Amber.Fbx`, the protected converter uses
the pinned local Blender, maps CC facial shapes to ARKit-52, trims unseen geometry, compacts dense
morph data, and installs the result without executing archive content:

```powershell
scripts\setup-audio2face-cc3-avatar.ps1 `
  -ArchivePath C:\path\Amber_Blender.rar -ConfirmAssetRights
```

The installer rejects external resources and scenes above 50 MB, 32 meshes, 500,000 vertices, 128
primitives, 80 morphs per mesh, 64 materials, 32 textures/images, 256 nodes, or 32 levels of node
depth. The browser never supplies an asset path. Optional tongue controls are used only when both
the installed Mark model and selected GLB provide matching morph targets.
